...
// ---- Contact enrichment (find the decision-maker for a lead) ----
// Used by lib/enrich.js with the web_search tool. STRICTLY public + verified;
// never guess or fabricate. Unknown fields come back as "".
export const enrichSchema = {
  ...
  required: ["decisionMaker", "title", "email", "phone", "linkedin"],
  additionalProperties: false,
};

export function enrichContactPrompt(module) {
  const who =
    module === "international"
      ? "the senior decision-maker who would commission outsourced software/IT work (e.g. CTO, VP Engineering, Head of Product/Procurement, or the relevant program/IT lead)"
      : "the owner / principal / decision-maker";
  return `You are a contact researcher. Using web search, find ${who} for the given organization, plus their PUBLICLY LISTED contact details.

Return JSON: { decisionMaker, title, email, phone, linkedin }.

STRICT RULES:
- Use ONLY information that is publicly published (company site, official directory, press release, public LinkedIn, government/tender contact page).
- NEVER guess or construct an email from a name+domain pattern. NEVER invent a phone number. If you cannot verify a field from a real public source, return an empty string "" for it.
- "email"/"phone": only a real, publicly-listed address/number (a role inbox like procurement@org is acceptable and useful). "linkedin": the public profile or company page URL if found, else "".
- "decisionMaker"/"title": the real person if findable; otherwise "" (do not put "Unknown").
- Prefer the most senior relevant, reachable contact. Keep every field concise. Output ONLY the JSON object.`;
}
