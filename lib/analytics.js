...
// Funnel over the normalized leads model (the new dashboard cards). Counts each
// lead once by how far it has progressed (lifecycle status).
export function leadFunnel(leads) {
  const advanced = new Set(["in_campaign", "messaged", "sent", "replied", "meeting_booked", "won", "lost"]);
  const meetingLike = new Set(["meeting_booked", "won"]);
  let found = leads.length, qualified = 0, inCampaign = 0, meetings = 0, won = 0;
  for (const l of leads) {
    if (l.qualified || l.status !== "new") qualified++;
    if (advanced.has(l.status)) inCampaign++;
    if (meetingLike.has(l.status)) meetings++;
    if (l.status === "won") won++;
  }
  return { found, qualified, inCampaign, meetings, won };
}
...
// Same funnel counts as unitEconomics, but spend/tokens come from the AUTHORITATIVE
// usage_events ledger (passed in) instead of campaigns_v2.usage_json (never written).
// Used by the Reports page so cost-per-outcome reflects real money spent.
export function unitEconomicsReal(campaigns, realSpend = 0, realTokens = 0) {
  const base = unitEconomics(campaigns);
  return {
    ...base,
    spend: realSpend,
    tokens: realTokens,
    costPerLead: base.leads ? realSpend / base.leads : 0,
    costPerQualified: base.qualified ? realSpend / base.qualified : 0,
    costPerWon: base.won ? realSpend / base.won : 0,
  };
}
