...
import { getSettings, insertUsageEvent, getApiKeys } from "./db";
...
const DEFAULT_CALL = { task: "other", campaignId: null, leadId: null, source: null, module: null };
...
// ---- Active provider ----
// Resolve an API key: env var wins (set at process start), DB-stored key is
// the in-app fallback for users who entered it via Settings → API Keys.
function resolveKey(envVar, dbGetter) {
  if (process.env[envVar]) return process.env[envVar];
  try { return dbGetter() || ""; } catch { return ""; }
}
function anthropicKey() { return resolveKey("ANTHROPIC_API_KEY", () => getApiKeys().anthropicKey); }
function geminiKey()    { return resolveKey("GEMINI_API_KEY",   () => getApiKeys().geminiKey); }
...
function provider(prefer) {
  ...
  if (want === "gemini" && !geminiKey() && anthropicKey()) return "anthropic";
  if (want === "anthropic" && !anthropicKey() && geminiKey()) return "gemini";
  return want;
}
...
export function getClient(prefer) {
  if (provider(prefer) === "gemini") {
    if (!geminiKey()) {
      throw new Error("GEMINI_API_KEY is not set. Add it to .env.local or via Settings → API Keys.");
    }
    return geminiClient();
  }
  // Anthropic (default)
  ...
  if (!aKey) {
    throw new Error(
      "ANTHROPIC_API_KEY is not set. Add it to .env.local or via Settings → API Keys."
    );
  }
  if (!rawClient || rawClient.apiKey !== aKey) {
    rawClient = new Anthropic({ apiKey: aKey });
    wrapped = null; // invalidate cached wrapper when key changes
  }
...
export function isMockMode(prefer) {
  ...
  if (provider(prefer) === "gemini") return !geminiKey();
  const aKey = anthropicKey();
  return (
    !aKey ||
    aKey.startsWith("sk-ant-api03-8f5n")
  );
}
