...
export function generateMockQualify(domain, highLeads) {
  ...
  return highLeads.map((lead, idx) => {
    ...
    return {
      ...
      exactGap: lead.gap,
      decisionMaker,
      // Non-actionable placeholders in mock mode: these are intentionally NOT a
      // valid number/email so toDigits()/isEmail() return null and every Send
      // button stays disabled — you must never message a fabricated contact.
      whatsapp: "Simulated — add API key",
      email: "Simulated — add API key",
